...
public class VancouverBusApp {
	...
	private void onSearchNameButtonPress() {
		...
		// Handles case sensitivity of search
		String search = searchNameTextField.getText().toUpperCase();
		if (search.isEmpty()) {
			println("Invalid search query!");
			return;
		}
		println("Searching for bus stops named '" + search + "'...");
		List<BusStop> foundStops = stops.getSearchResults(search);

		// Print results
		if (foundStops.isEmpty())
			println("No Search Results Found.");
		for (BusStop stop : foundStops)
			println(
				"Stop #" + stop.getID() + ": " + stop.getName() +
				" - Location: (" + stop.getLatitude() + ", " + stop.getLongitude() + ")");
	}
