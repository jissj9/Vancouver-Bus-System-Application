...
public class BusFileParser {
	...
	// Let the user select a file before attempting to parse the file for bus stop times
	...
	// Let the user select a file before attempting to parse the file for bus stop transfers
	...
	public static Map<Integer, BusStop> LoadStops(File file, boolean printErrors) throws IOException {
		...
		while ((currentLine = reader.readLine()) != null) {
			try {
				...
				stops.put(stopID, new BusStop(
					stopID,
					data[1].isEmpty() ? 0 : Integer.parseInt(data[1]),
					data[2],
					data[3],
	...
	public static int LoadStopTimes(BusStops stops, File file, boolean printErrors) throws IOException {
		...
		while ((currentLine = reader.readLine()) != null) {
			try {
				...
				// Only add a trip to stop from the previous stop if this is the same trip as the last one
				if (tripID == previousTripID) {
					previousStop.addTrip(new BusTrip(
						tripID,
						LocalTime.parse(data[1], STOP_TIME_EXPECTED_TIME_FORMAT),
						LocalTime.parse(data[2], STOP_TIME_EXPECTED_TIME_FORMAT),
						previousStop,
						stop,
						data[4].isEmpty() ? 0 : Integer.parseInt(data[4]),
						data[5],
